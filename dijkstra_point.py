...
def generate_obstacles():
    ...
    for x in range(301):
        for y in range(201):
            ...
            # ellipse
            if ((x - 150)/40)**2 + ((y - 100)/20)**2 <= 1:
                obstacle_space.append([x,y])
            # rhombus
            if (5*y - 3*x + 5*95 <= 0) and (5*y + 3*x - 5*175 <= 0) and \
                    (5*y - 3*x + 5*125 >= 0) and (5*y + 3*x - 5*145 >= 0):
                obstacle_space.append([x,y])
            # rectangle
            if (5*y - 9*x - 65 <= 0) and (65*y + 37*x - 5*1247 <= 0) and \
                    (5*y - 9*x + 5*141 >= 0) and (65*y + 37*x - 5*1093 >= 0):
                obstacle_space.append([x,y])
            # polygon
            if (y - 185 <= 0) and (5*y + 7*x - 5*290) <= 0 and \
                    (5*y - 6*x - 150 >= 0) and (5*y + 6*x - 5*210 >= 0) and (5*y + 7*x - 5*220 >= 0):
                obstacle_space.append([x,y])
            if (y - x - 100 >= 0) and (y - 13*x + 140 <= 0) and (5*y + 7*x - 5*220 <= 0):
                obstacle_space.append([x,y])
...
#Main program
Take_Input()
x_obs,y_obs,obstacle_space,Final_obs = generate_obstacles()
